 ...
 * @author Utilizador
 */
public class CosineContentSimilarity {
        ...
        for (int i = 0; i < columnsbdw.length; i++) {
            ...
            for (int j = 0; j < columnsNS.length; j++) {
                ...
                Double similarityMesure = javapair.map(pair -> {
                    ...
                    return cosineSim.similarity(pair._1.mkString(), pair._2.mkString());
                }).reduce(((c1, c2) -> c1 + c2));
                ...
                Score scoreSimilarity = new Score(similarityMesure, (double) endCosine.getEpochSecond());
