...
import hesml.taxonomyreaders.wordnet.impl.WordNetDB;
import hesml.taxonomyreaders.wordnet.impl.WordNetReader;
import hesml.taxonomyreaders.wordnet.impl.WordNetSynset;
...
public class HESMLclient {
     ...
     * use them in your experiments, you should cite the origional works where
    ...
    /**
     * Miller, G. A., and Charles, W. G. (1991). Contextual correlates of
    ...
    /**
     * Agirre, E., Alfonseca, E., Hall, K., Kravalova, J., Paşca, M., and
     * Soroa, A. (2009). A Study on Similarity and Relatedness Using
     * Distributional and WordNet-based Approaches. In Proceedings of Human
     * Language Technologies: The 2009 Annual Conference of the North American
     * Chapter of the Association for Computational Linguistics (pp. 19–27).
     * Stroudsburg, PA, USA: Association for Computational Linguistics.
     */
    private static final String AGIRRE203 = "Agirre201_dataset";

    /**
     * Hill, F., Reichart, R., and Korhonen, A. (2014). SimLex-999: Evaluating
     * Semantic Models with (Genuine) Similarity Estimation. arXiv:1408.3456.
     * Retrieved from http://arxiv.org/abs/1408.3456
     */
    private static final String SIMLEX665 = "SimLex665_dataset";

    /**
     * WordNet database path, corpus-based Pedersen IC files, and results
     * directory in the current PC. You should change the base HESML directory
     * according to your code installation.
     */
    private static final String m_strBaseDir = "..";
    private static final String m_strWordNetDatasetsDir = m_strBaseDir + "/WN_Datasets/";
    private static final String m_strWordNet3_0_Dir = "src\\main\\resources\\Wordnet-3.1\\dict";
    private static final String m_strResultsDir = m_strBaseDir + "/ReproducibleExperiments/";
    private static final String m_strCorpusFrqFilesDir = "src\\main\\resources\\PedersenICmodels\\WordNet-InfoContent-3.0\\";
    ...
    /**
     * This is the main function where we show different use examples of HESML.
     * The function processes an input XML file defining a set of reproducible
     * experiments, or call a function implementing a set of examples in order
     * to show the HESML functionality.
     *
    ...
    /**
     * Each function call within this function is invoking some specific test
     * example with the aim to show the HESML functionality.
     */
...
//        // We cretae the database       
        ...
        // System.out.println(contentDateLots);
...
//        testSingleICSimMeasureSingleICmodel();
    ...
    /**
     * This function runs the full collection of cross-validation tests for all
     * the corpus-based IC models. The function runs the experiments on the five
     * most significant similarity datasets.
     *
     * The function loads WordNet and creates a HESML taxonomy to represent it.
     * The benchmarks loads the dataset files with the word pairs and creates a
     * experiment matrix to evaluate each selected similarity measures and IC
     * model.
     *
     * @throws Exception
     */
    ...
    /**
     * This function runs the full collection of cross-validation tests for all
     * the IC models and similarity measures, including the corpus-based and
     * intrinsic IC models, as well as the non IC-based similarity measures. The
     * function runs the experiments on the five most significant similarity
     * datasets.
     *
     * The function loads WordNet and creates a HESML taxonomy to represent it.
     * The benchmarks loads the dataset files with the word pairs and creates a
    ...
    /**
     * This function runs the tests for all the intrinsic IC models and IC-based
     * similarity measures. The function runs the experiments on the five most
     * significant similarity datasets.
     *
     * The function loads WordNet and creates a HESML taxonomy to represent it.
     * The benchmarks loads the dataset files with the word pairs and creates a
     * experiment matrix to evaluate each selected similarity measures and IC
    ...
    /**
     * This function evaluates a single non IC-based similarity measure on a
     * word similarity benchmark using an automatized benchmark object. You only
     * need to load WordNet and select any benchmark. The benchmark computes the
    ...
    /**
     * This function corresponds to the cross-validation of a single IC-based
     * similarity measure with multiple intrinsic IC models on a word similarity
     * benchmark (dataset) using an automatized benchmark object. The benchmark
     * computes the Pearson correlation values and save them into a csv file.
     *
     * @throws Exception
     */
    ...
    private static void testWordPairSimilarity() throws Exception {
        ...
        ISimilarityMeasure LIN;     // Sánchez et al (2012)
        ...
        word1Concepts = wordnetTaxonomy.getVertexes().getByIds(
                wordnet.getWordSynsetsID("sale"));

        word2Concepts = wordnetTaxonomy.getVertexes().getByIds(
                wordnet.getWordSynsetsID("sales"));
...
//        secoICmodel.setTaxonomyData(wordnetTaxonomy);
        ...
        LIN = MeasureFactory.getMeasure(wordnetTaxonomy, SimilarityMeasureType.Lin);
        ...
        simValues[3] = LIN.getHighestPairwiseSimilarity(word1Concepts, word2Concepts);

        System.out.println(simValues[0]);
        System.out.println(simValues[1]);
        System.out.println(simValues[2]);
        System.out.println(simValues[3]);
     ...
     * is introduced in the following paper. Lastra-Díaz, J. J., and
     * García-Serrano, A. (2016). WNSimRep: a framework and replication dataset
     * for ontology-based semantic similarity measures and information content
     * models. Mendeley Data v1. https://doi.org/10.17632/mpr2m8pycs.1
     */
    ...
    /**
     * This function copies the Synset words into the string tag field. It is
     * used by the WNSimRep functions in order to export the words evocating
     * each synset to the data files.
     *
     ...
     * @param strWordnetDatasetsDir Input directory for the word similarity
     * benchmarks
    ...
    /**
     * This functions returns the sequence of intrinsic IC models that will be
     * evaluated in the benchmarks.
     *
     * @return
     */
    ...
    /**
     * This function returns the measure types to be evaluated into the IC-based
     * benchmarks. You can set another measures according to the experiments
    ...
    /**
     * This function returns the measure types to be evaluated into the non
     * IC-based benchmarks. You can set another measures according to the
     * experiments that you need to carry-out.
     *
     * @return Vector of IC-based measure types.
     */
     ...
     * @param strWordnetDatasetsDir Directory containing the word similarity
     * benchmarks
    ...
    /**
     * This function creates a tree-like taxonomy with the number of vertexes
     * defined by the input parameter. For each vertex (node) of the tree, the
     * function inserts a random number of children between two and maximal
     * value. The seed of the generator of random numbers is always the same in
